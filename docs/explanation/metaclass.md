...
::: info 0.3 修复：尊重显式 `sa_type`
0.3.0 起，Python 3.14 兼容补丁修复了一个隐患：原本在 PEP 649 路径下，用户显式写的 `Field(sa_type=...)` 会被默认推断覆盖。修复后元类会先检查 `sa_type` 是否已经被显式设置，只有未设置时才注入推断值。
:::
...
原函数遇到 `ForwardRef`、`ClassVar`、`Literal[StrEnum.MEMBER]` 等类型时调用 `issubclass()` 导致 `TypeError`。补丁在调用前拦截这些特殊情况，并尊重用户显式设置的 `Field(sa_type=...)`。
