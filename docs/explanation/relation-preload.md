...
在异步 SQLAlchemy 中，访问未加载的关系会触发隐式同步查询 → `MissingGreenlet`。常规解决办法是调用方加 `load=` 预加载，但**这要求调用方知道方法内部用了哪些关系**——这是漏洞百出的契约。

`@requires_relations` 把"我需要哪些关系"声明在方法本身，调用方什么都不用管。本章讲它**怎么实现**这个魔法；要看怎么用它，去 [防止 MissingGreenlet 错误](/how-to/prevent-missing-greenlet)。
