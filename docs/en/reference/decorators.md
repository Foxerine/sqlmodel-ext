...
| Member | Description |
|------|------|
| `commit(*, fail_soft_when_observed=False)` | Changes to cached models (including bare `session.add()` / attribute changes / `session.delete()`) are registered by the flush that writes them -- earlier in the transaction or during this commit; after commit, invalidates **synchronously**, then runs the post-commit callbacks in order. `fail_soft_when_observed=True`: a failure of the commit itself is raised as usual; every step **after** the commit (invalidation, each callback) is individually fault-tolerant (catches `BaseException` including cancellation, logs, continues). Default: errors raised by the invalidation step and cancellation propagate; a callback's `Exception` is logged and skipped. **Redis failures during invalidation are logged, not raised, in both modes** — the database has committed, so the affected cache entries serve pre-commit data until their TTL expires; use `no_cache=True` for reads that must not rely on the cache |
