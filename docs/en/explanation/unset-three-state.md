...
- field-level attributes (`exclude` / `alias` / `validation_alias` / `serialization_alias` / `discriminator` / `repr` / `frozen`) are hoisted outside the union — otherwise Pydantic would silently drop them
