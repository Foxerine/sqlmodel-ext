...
5. 有错误时，再在**未展开**的副本上跑一次作为基线，只报告展开**新增**的错误。比对键是 `(文件, 规则, 消息, 报错所在行的文本)` 的计数，**不含行号**——展开会插入代码行，含行号会把每一条既有错误都算成"新增"。行文本随代码一起移动，所以既有错误仍能对上，而同规则同消息、出现在另一行的新错误照样会被报告（若不含行文本，同一文件里一条旧错误消失、一条新错误出现会相互抵消，新错误被隐藏）。残余限制：同一文件里文本完全相同的两行上的两条相同错误，对基线而言可互换。
6. 删除临时目录（`--keep` 时保留）。

**工具本身从不向你的项目写入**：所有写入都在临时副本里；导入你的模块时关闭了字节码写入，连 `__pycache__` 都不会出现；写入函数拒绝把副本放在项目目录之内。仓库里不会有任何生成物。

::: warning 你的模块会被导入
为了读取运行时事实，目标模块会**从你的项目中被导入**——和跑测试时导入它们一样——所以模块顶层的副作用会执行：导入时就写文件、开连接或启动服务的模块，照样会这么做。只对可安全导入的模块使用 `check_derived`（模型定义通常都是）。
:::
