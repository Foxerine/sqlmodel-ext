...
features:
  ...
  - title: 丰富的字段类型
    details: Str64、Port、HttpUrl、SafeHttpUrl、IPAddress、Array[T]、高精度 Decimal（NUMERIC 列 + 定点 JSON 字符串序列化）等，Pydantic 验证 + SQLAlchemy 列类型一步到位
  - title: 多态继承
    details: 联表继承 (JTI) 和单表继承 (STI) 的零配置支持，自动鉴别列与子类注册，DeferredIndex 支持基类声明子类列索引
  - title: Redis 查询缓存
    details: CachedTableBaseMixin 提供 ID 缓存 + 查询缓存双层架构，增强 AsyncSession 在 commit 时自动同步失效，支持多态继承
  - title: 安全与可靠
...
底层 `select()`、查询构造、迁移工具仍是原生 SQLAlchemy，没有自创 DSL，也不接管 `engine` 的生命周期。唯一的 session 层增强是可选的 `sqlmodel_ext.AsyncSession`（sqlmodel `AsyncSession` 的薄子类，缓存场景用 `async_sessionmaker(class_=AsyncSession)` 接入即可）。换句话说：在已有 SQLModel/SQLAlchemy 项目里，可以单独引入需要的 Mixin，而不必重写既有数据访问层。
